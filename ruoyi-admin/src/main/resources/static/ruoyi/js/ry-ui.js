...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            //下载模版自定义 客户列表模版
            importTemplatecuslist: function() {
        ...
        operate: {
            ...
            removefile: function(id) {
                table.set();
                $.modal.confirm("确定删除该条" + table.options.modalName + "信息,以及云端数据吗？", function() {
                    var url = $.common.isEmpty(id) ? table.options.removeUrl : table.options.removeUrl.replace("{id}", id);
                    if (table.options.type == table_type.bootstrapTreeTable) {
                        $.operate.get(url);
                    } else {
            ...
            downloadFile: function() {
                ...
                if (rows.length > 1) {
                    $.modal.alertWarning("您只能选择 1 个文件下载");
                    return;
                }
                $.modal.confirm("确认要下载选中的云端数据吗?", function() {
            ...
            conformout: function() {
                ...
                $.modal.confirm("确定审核该条出库单数据吗?", function() {
                    var url = table.options.confirmoutorder;
                    var data = { "ids": rows.join() };
                    $.operate.submit(url, "post", "json", data);
                });
            },
            ...
            fconform: function() {
                ...
                $.modal.confirm("确定反审核该条出库单数据吗?", function() {
                    var url = table.options.fconfirmoutorder;
                    var data = { "ids": rows.join() };
                    $.operate.submit(url, "post", "json", data);
                });
            },
